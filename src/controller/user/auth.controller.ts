...
const sendSignupOtp = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if(user) return res.status(409).send({ status: 409, msg: 'A user with this email already  exist, please login'});
...
const login = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if(!isVerified) return res.status(403).send({ status: 403, message: 'Otp not valid or has expired' });
